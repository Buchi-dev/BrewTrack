...
with ordered_stations as (
  select id, row_number() over (order by name, code) as station_number
  from public.branches
  where is_active
)
update public.station_schedule_assignments schedule
set branch_id = ordered_stations.id
from ordered_stations
where schedule.branch_id is null
  and schedule.station_number = ordered_stations.station_number;

drop index if exists station_schedule_assignments_station_idx;
...
alter table public.station_schedule_assignments
  drop constraint if exists station_schedule_assignments_station_check;

drop trigger if exists validate_station_schedule_assignment_before_write
  on public.station_schedule_assignments;

drop trigger if exists protect_station_schedule_trainer_links_before_update_delete
  on public.station_schedule_assignments;
