...
export default function DailySchedule({ data, demo, onChange, notify, readOnly = false }) {
  ...
  return <>
    ...
    <div className={`schedule-layout ${readOnly ? `schedule-read-only employee-${employeeView}` : ''}`}>
      <section className="panel month-panel" aria-label={monthLabel(month)}><div className="panel-heading"><div><h2>{monthLabel(month)}</h2><p>{readOnly ? 'Your assignments' : `${stationInfo?.name || 'Select a station'} · Core coverage and training staff`}</p></div><CalendarDays size={22}/></div>
