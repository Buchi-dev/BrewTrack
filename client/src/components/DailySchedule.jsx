...
export default function DailySchedule({ data, demo, onChange, notify, readOnly = false }) {
  ...
  return <>
    {readOnly && <section className="panel next-shift-card"><div><span className="section-label"><span className="live-dot"/>Next shift</span>{nextAssignment ? <><h2>{dayLabel(nextAssignment.work_date)}</h2><p><MapPin size={15}/>{stationLabel(data, nextAssignment)}</p>{nextAssignment.work_role === 'Trainee' && <p className="next-shift-trainer">Trainer: {trainerName(data, nextAssignment)}</p>}</> : <><h2>No upcoming shift</h2><p><CalendarDays size={15}/>Your manager has not added a future assignment yet.</p></>}</div>{nextAssignment && <div className="next-shift-meta"><Role role={nextAssignment.work_role}/><span><Clock3 size={14}/>{shiftTime(nextAssignment)}</span></div>}</section>}
    <div className="schedule-controls"><MonthControl month={month} onChange={changeMonth}/>{!readOnly && <select aria-label="Schedule station" value={station} onChange={e => setStation(e.target.value)}>{!data.stations.length && <option value="">No stations</option>}{data.stations.map(s => <option key={s.id} value={s.id}>{s.name}{!s.active ? ' (inactive)' : ''}</option>)}</select>}<button className="button" onClick={() => changeMonth(today.slice(0, 7))}>Today</button><button className="button" onClick={exportSchedule}><ArrowDownToLine size={16}/>Export schedule</button>{readOnly && <div className="schedule-view-toggle" aria-label="Schedule view"><button className={employeeView === 'list' ? 'active' : ''} onClick={() => setEmployeeView('list')}><List size={15}/>List</button><button className={employeeView === 'calendar' ? 'active' : ''} onClick={() => setEmployeeView('calendar')}><CalendarDays size={15}/>Calendar</button></div>}</div>
    <div className="schedule-legend"><span className="legend-label">Legend</span>{WORK_ROLES.map(role => <Role key={role} role={role}/>)}<span className="legend-locked"><LockKeyhole size={13}/> {readOnly ? 'Locked schedule' : 'Locked schedule: past dates or attendance'}</span></div>
    {error && <div className="error" role="alert">{error}</div>}
    {readOnly && <section className={`panel upcoming-panel ${employeeView === 'list' ? 'active' : ''}`}><div className="panel-heading"><div><h2>Upcoming shifts</h2><p>{upcomingAssignments.length ? `${upcomingAssignments.length} shift${upcomingAssignments.length === 1 ? '' : 's'} ahead` : 'No upcoming shifts'}</p></div><List size={20}/></div><div className="upcoming-list">{upcomingAssignments.map(a => <button key={a.id} onClick={() => { setDate(a.work_date); setMonth(a.work_date.slice(0, 7)) }}><strong>{dayLabel(a.work_date)}</strong><span>{stationLabel(data, a)}</span><Role role={a.work_role}/>{a.work_role === 'Trainee' && <small>Trainer: {trainerName(data, a)}</small>}<small><Clock3 size={13}/>{shiftTime(a)}</small></button>)}{!upcomingAssignments.length && <div className="schedule-empty"><strong>No upcoming shifts</strong><p>Your manager has not added a future assignment yet.</p></div>}</div></section>}
    <div className={`schedule-layout ${readOnly ? `schedule-read-only employee-${employeeView}` : ''}`}>
      <section className="panel month-panel" aria-label={monthLabel(month)}><div className="panel-heading"><div><h2>{monthLabel(month)}</h2><p>{readOnly ? 'Your schedule' : `${stationInfo?.name || 'Select a station'} · Core coverage and training staff`}</p></div><CalendarDays size={22}/></div>
        <div className="calendar-scroll"><div className="month-grid">{weekdays.map(day => <div className="weekday" key={day}>{day}</div>)}{Array.from({ length: offset }, (_, i) => <div className="calendar-blank" key={`blank-${i}`} aria-hidden="true"/>)}{days.map(day => {
          ...
          const core = team.filter(a => CORE_ROLES.includes(a.work_role))
          const trainees = team.filter(a => a.work_role === 'Trainee')
          ...
          return <div className={`calendar-day ${day === date ? 'selected' : ''} ${day < today ? 'past' : ''}`} key={day} onDragOver={e => { if (!readOnly && !busy && day >= today && stationInfo?.active) e.preventDefault() }} onDrop={e => { e.preventDefault(); const id = e.dataTransfer.getData('application/x-brewtrack-staff'); if (data.employees.some(staff => staff.id === id && staff.active)) assign(id, day) }}>
            ...
            <div className="calendar-team">{!readOnly && core.length > 0 && <div className="assignment-group-label">Core staff</div>}{core.map(a => <button key={a.id} className="calendar-assignment" onClick={() => { setDate(day); if (!readOnly && !busy && !assignmentLocked(a, data.records, today)) setEditing(a) }} aria-label={readOnly ? `${stationLabel(data, a)}, ${a.work_role}${a.trainer_id ? `, trainer ${trainerName(data, a)}` : ''}, ${shiftTime(a)}` : `${employeeName(data, a)}, ${a.work_role}${assignmentLocked(a, data.records, today) ? ', locked' : ', edit assignment'}`}><span>{readOnly ? stationLabel(data, a) : employeeName(data, a)} {assignmentLocked(a, data.records, today) && <LockKeyhole size={10}/>}</span><Role role={a.work_role}/>{readOnly && <small><Clock3 size={11}/>{shiftTime(a)}</small>}</button>)}{!readOnly && trainees.length > 0 && <div className="assignment-group-label training-label">Training staff</div>}{trainees.map(a => <button key={a.id} className="calendar-assignment trainee-assignment" onClick={() => { setDate(day); if (!readOnly && !busy && !assignmentLocked(a, data.records, today)) setEditing(a) }} aria-label={`${employeeName(data, a)}, Trainee, trainer ${trainerName(data, a)}${assignmentLocked(a, data.records, today) ? ', locked' : ', edit assignment'}`}><span>{readOnly ? stationLabel(data, a) : employeeName(data, a)} {assignmentLocked(a, data.records, today) && <LockKeyhole size={10}/>}</span><Role role="Trainee"/><small>Trainer: {trainerName(data, a)}</small>{readOnly && <small><Clock3 size={11}/>{shiftTime(a)}</small>}</button>)}{readOnly && !team.length && <p className="calendar-empty">No shift</p>}</div>
            {!readOnly && <div className={`coverage ${!missing.length ? 'covered' : ''}`}><strong>{core.length} / 3 core staff</strong><span>{missing.length ? `Needs ${missing.join(', ')}` : 'Core coverage complete'}</span>{trainees.length > 0 && <em>{trainees.length} training staff</em>}</div>}
          </div>
        })}</div></div>{readOnly && !monthly.length && <div className="schedule-empty month-empty"><strong>No schedule this month</strong><p>You do not have any assignments in this month.</p></div>}
      </section>
    ...
    <section className="panel selected-day"><div className="panel-heading"><div><h2>{dayLabel(date)}{!readOnly && ` · ${stationInfo?.name || 'Station'}`}</h2><p>{readOnly ? (visible.length ? 'Shift details' : 'No schedule for this day') : `${visible.length} staff assigned${date < today ? ' · Past schedule locked' : ''}`}</p></div>{!readOnly && <div className="schedule-day-actions"><button className="button" disabled={busy || !stationInfo?.active || !visible.length} onClick={() => setCopyMode('day')}><Copy size={14}/>Copy day</button><button className="button" disabled={busy || !stationInfo?.active} onClick={() => setCopyMode('week')}><Copy size={14}/>Copy week</button><button className="button primary" disabled={busy || date < today || !stationInfo?.active} onClick={() => assign()}><Plus size={15}/>Assign staff</button></div>}</div>
      <div className="selected-team">{!readOnly && <div className="selected-coverage"><div><strong>Core coverage</strong><span>{visible.filter(a => CORE_ROLES.includes(a.work_role)).length} / 3</span></div>{CORE_ROLES.map(role => { const assignment = visible.find(a => a.work_role === role); return <div className="coverage-row" key={role}><span>{assignment ? '✓' : '○'} {role}</span><strong>{assignment ? employeeName(data, assignment) : 'Needs coverage'}</strong></div> })}</div>}{!readOnly && visible.some(a => a.work_role === 'Trainee') && <div className="selected-coverage training-summary"><div><strong>Training</strong><span>{visible.filter(a => a.work_role === 'Trainee').length}</span></div></div>}{visible.map(a => <div className={`selected-person ${a.work_role === 'Trainee' ? 'selected-trainee' : ''}`} key={a.id}>{readOnly ? <div className="employee-detail"><div><span>Station</span><strong>{stationLabel(data, a)}</strong></div><div><span>Role</span><Role role={a.work_role}/></div>{a.work_role === 'Trainee' && <div><span>Trainer</span><strong>{trainerName(data, a)}</strong></div>}<div><span>Shift</span><strong><Clock3 size={12}/>{shiftTime(a)}</strong></div>{date < today && <small className="locked-note"><LockKeyhole size={12}/>Locked schedule</small>}</div> : <><strong>{employeeName(data, a)}</strong><Role role={a.work_role}/>{a.trainer_id && <span className="muted">Trainer: {trainerName(data, a)}</span>}{assignmentLocked(a, data.records, today) ? <span className="muted"><LockKeyhole size={12}/> Locked</span> : <div className="schedule-actions"><button className="text-button" disabled={busy} onClick={() => setEditing(a)}>Edit</button><button className="text-button" disabled={busy} onClick={() => remove(a)}>Remove</button></div>}</>}</div>)}{!visible.length && <div className="schedule-empty"><strong>No schedule for this day</strong><p>You do not have an assignment on this date.</p></div>}</div>
    </section>
...
function AssignmentForm({ assignment, data, demo, onClose, onSave }) {
  ...
  return <Modal title={assignment.id ? 'Edit assignment' : 'Assign staff'} subtitle={`${dayLabel(assignment.work_date)} · Philippine time`} onClose={() => { if (!busy) onClose() }}><form onSubmit={submit}><fieldset className="schedule-form-fields" disabled={busy}><label className="field">Staff<select name="employee_id" required disabled={!!assignment.id} value={employeeId} onChange={e => { const next = data.employees.find(item => item.id === e.target.value); setEmployeeId(e.target.value); setRole(next?.staff_type === 'trainee' ? 'Trainee' : '') }}><option value="" disabled>Select staff</option>{data.employees.filter(e => e.active || e.id === assignment.employee_id).map(e => <option key={e.id} value={e.id}>{e.name}{e.staff_type === 'trainee' ? ' · Trainee' : ''}</option>)}</select></label><label className="field">Station<select name="station_id" required value={stationId} onChange={e => setStationId(e.target.value)}><option value="" disabled>Select station</option>{data.stations.filter(s => s.active).map(s => <option key={s.id} value={s.id}>{s.name}</option>)}</select></label><label className="field">Work role<select name="work_role" required value={role} disabled={employee?.staff_type === 'trainee'} onChange={e => setRole(e.target.value)}><option value="" disabled>Select role</option>{(employee?.staff_type === 'trainee' ? ['Trainee'] : CORE_ROLES).map(item => <option key={item}>{item}</option>)}</select>{employee?.staff_type === 'trainee' && <small className="form-footnote">Role is automatically set to Trainee.</small>}</label>{role === 'Trainee' && <label className="field">Trainer<select name="trainer_id" required defaultValue={assignment.trainer_id || ''}><option value="" disabled>Select trainer from today’s core staff</option>{trainers.map(e => <option key={e.id} value={e.id}>{e.name}</option>)}</select><small className="form-footnote">Trainer must be assigned to this station on this day.</small></label>}{!assignment.id && <DatePicker dates={dates} onChange={setDates} initialMonth={assignment.work_date.slice(0, 7)}/>}<p className="form-footnote">Past dates and attendance are locked. Each employee can have only one assignment per day across all stations.</p></fieldset>{error && <div className="error" role="alert">{error}</div>}<div className="modal-actions"><button type="button" className="button" disabled={busy} onClick={onClose}>Cancel</button><button className="button primary" disabled={busy || !dates.length}>{busy ? <LoaderCircle className="spin" size={16}/> : <Check size={16}/>}Save {assignment.id ? 'assignment' : `${dates.length} assignment${dates.length === 1 ? '' : 's'}`}</button></div></form></Modal>
}
